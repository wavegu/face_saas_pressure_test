...
gConfig['sample jmx path'] = 'sample_jmx.jmx'
...
gConfig['test report path'] = 'test_report/'
...
class Jmeter:
	...
	def GenerateConfigXmlFile(self):
		global gConfig
		sampleFile = open(gConfig['sample jmx path'])
	...
	def Terminate(self):
		...
		os.system('bash bin/shutdown.sh')

	def Run(self):
		self.GenerateConfigXmlFile()
		self.mMonitor.start()
		print self.mRequestBodyCluster
		startCmd = '../bin/jmeter -n -t ' + self.mConfigXmlFile + ' -l ' + self.mSampleLogFile
		os.system(startCmd)
...
class TestController:
	...
	def RunTest(self):
		...
		for caseName in self.mTestCaseDic.keys():
			for threadNumber in range(gConfig['thread number min'], gConfig['thread number max'] + 1, gConfig['thread number step']):
				colorPrint('b', caseName + ' with ' + str(threadNumber) + ' threads begins....')
				jmeter = Jmeter(threadNumber, caseName)
				print 'generate jmeter succ'
				jmeter.GenerateConfigXmlFile()
				print 'GenerateConfigXmlFile succ'
				jmeter.Run()
	...
	def GetReport(self):
		...
		if not os.path.isdir(gConfig['test report path']):
			os.mkdir(gConfig['test report path'])
		report = Report(gConfig['test report path'] + gConfig['test name'].replace(' ', '_') + '.rep')
